 ...
 */
static PurpleCmdId log_command_id, fortune_id;
...
{
  purple_debug_misc(PLUGIN_ID, "fortune_cb called\n");
...
/**
 * The callback function for our /fortune command. This function simply pops up
...
fortune_cb(PurpleConversation *conv, const gchar *cmd, gchar **args, gchar **error, void *data)
{
  ...
  //                   "Fortune Notification Title", /* window header */
  //                   "Fortune Notification Primary Text",   /* text displayed */
  //                   args[0]);   /* Secondary text is the word entered */
  //                   
  gchar *msg;
  
  msg = g_strdup_printf("Fortune: ");
...
plugin_load(PurplePlugin *plugin)
{
  ...
  // Help message for fortune command, in the correct format
  fortune_help = "fortune: Prints fortune from gnu linux in current channel.";

  // Register a command to allow a user to enter /fortune and have
  ...
  fortune_id = purple_cmd_register
    ("fortune",                     /* command name */ 
     NULL,                         /* "w" command argument format */
...
plugin_unload(PurplePlugin *plugin)
{
  ...
  purple_cmd_unregister(fortune_id);
...
static PurplePluginInfo info = {
        ...
        "Fortune",      /* name */
        "0.1",                      /* version */
        "Print fortune from Gnu Linux",      /* summary */
        "Print fortune from Gnu Linux",      /* description */
